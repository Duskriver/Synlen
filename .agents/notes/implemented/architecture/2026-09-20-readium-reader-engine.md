...
进度直接采用 `BookProgress` 的完整 Locator JSON，保留文本上下文与 SDK 扩展字段，百分比只供书架展示。存量旧坐标使用独立格式保留，兼容恢复遵循[升级保留数据决策](../bug-fix/2026-09-21-preserve-legacy-reader-data.md)；备份保存同一份定位数据。
...
完整 Locator 原样落库，阅读百分比按章节等权估算，不能用于恢复。数据库 schema 1、2 升级到 3 时保留旧坐标，首次近似恢复成功后才保存完整 Locator；升级决策部分取代本笔记的不迁移前提。备份兼容由同一决策持有。
