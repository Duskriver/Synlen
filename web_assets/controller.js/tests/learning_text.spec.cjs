...
const bundle = buildSync({ entryPoints: [path.join(__dirname, '../renderer/learning_text.ts')], bundle: true,
  write: false, format: 'iife', globalName: 'LearningText', target: 'es2015' }).outputFiles[0].text;
const interactionBundle = buildSync({ entryPoints: [path.join(__dirname, '../renderer/interaction.ts')], bundle: true,
  write: false, format: 'iife', globalName: 'Interaction', target: 'es2015' }).outputFiles[0].text;
...
test('真实 iframe 中点词和长按复用上下文，空白处不弹查词', async ({ page }) => {
  await page.setContent('<iframe style="width:600px;height:300px" srcdoc="<p style=\'font-size:24px;line-height:2\'>I <em>really</em> love this book.</p>"></iframe>');
  await page.addScriptTag({ content: interactionBundle });
  const result = await page.evaluate(async () => {
    const frame = document.querySelector('iframe');
    if (!frame.contentDocument.querySelector('em')) await new Promise(resolve => frame.onload = resolve);
    const calls = [];
