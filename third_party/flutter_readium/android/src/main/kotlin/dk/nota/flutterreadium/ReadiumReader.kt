...
object ReadiumReader :
    ...
    suspend fun openPublication(pubUrl: AbsoluteUrl): Try<Publication, PublicationError> {
        ...
        val publicationInjections = javaScriptInjections + cssInjections
                ...
                return TransformingContainer(container) { url: Url, resource: Resource ->
                    ...
                    if (isHtml) {
                        resource.injectScriptsAndStyles(
                            tocIds,
                            epubPreferences,
                            publicationInjections,
                            navigator.localFontStyleSheet(),
                        )
                    } else {
                        resource
                    }
    ...
    fun visualClose() {
        val reader = currentReaderWidget
        val navigator = visualNavigator
        navigator?.dispose()
        if (visualNavigator === navigator) visualNavigator = null
        if (currentReaderWidget === reader) currentReaderWidget = null
    }
