...
`book_import_service.dart` 568 行：导入流水线（去重、格式分发、解析、落库、回滚）与三类自成一体的能力混在一个类里——ZIP 魔数嗅探与哈希、书籍文件落盘、封面提取（含 OPF 相对路径解析与压缩）。后三者各自有明确的输入输出，却只能通过整条导入流水线验证。
...
- `book_import_service.dart` 由 568 行降到 399 行，library 的超 400 行清单少一项。
