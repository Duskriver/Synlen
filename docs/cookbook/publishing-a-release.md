...
3. 提交并打 tag：`git tag vX.Y.Z && git push origin vX.Y.Z`。`.github/workflows/build_release.yml` 的 `quality-gates` job 先跑 analyze、全量测试、文档门禁与生成物漂移，再构建 APK、AAB 与 iOS 产物。
...
5. 核对 `version.json`：`updateLog` 与 release notes 一致，`androidApkUrl` 指向本次 APK。
...
- 更新分发只走 OSS 的 `version.json`，`androidApkUrl` 必须是 universal APK；split APK 只作备用下载，不进 `version.json`。
