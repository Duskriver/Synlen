...
- **`docs/user/privacy.md`**：按"留在设备 / 离开设备"两张表列出数据流向：点词发单词 + 句子上下文给 DeepSeek、句子分析发原句、TTS 发文本与音色给阿里云、单词发音查 `dictionaryapi.dev`、更新检查只读 Gitee 清单；明确无账户、无遥测、无崩溃上报，备份为明文 ZIP 且不含密钥。
