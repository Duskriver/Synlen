...
class EpubReader {
  ...
  _expandToSentence(range) {
    ...
    while (start > 0) {
      ...
      if (isTerminator(char)) {
        // Found a potential end of PREVIOUS sentence.
        
        // 1. Check for "Mr.", "Dr.", etc.
        // Get 3 chars before dot to check for common abbreviations
        // We look back from start-1 (which is the dot)
        const precedingText = text.substring(Math.max(0, start - 5), start - 1); 
        if (/(Mr|Dr|Ms|Mrs|St|Vs)$/.test(precedingText)) {
           start--;
           continue;
        }
        
        // 2. Check for single capital letter (initials) e.g. "T. L."
        // text.charAt(start - 2) is the char before dot
        if (start >= 3 && /[A-Z]/.test(text.charAt(start - 2)) && text.charAt(start - 3) === ' ') {
           start--;
           continue;
        }
    ...
    while (end < text.length) {
      ...
      if (isTerminator(char)) {
        // Found potential end of CURRENT sentence.
        
        // 1. Check for abbreviations
        const precedingText = text.substring(Math.max(0, end - 4), end);
        if (/(Mr|Dr|Ms|Mrs|St|Vs)$/.test(precedingText)) {
           end++;
           continue;
        }

        // 2. Check for initials
        if (end >= 1 && /[A-Z]/.test(text.charAt(end - 1)) && (end < 2 || text.charAt(end - 2) === ' ')) {
           end++;
           continue;
        }
        ...
        // Include closing quotes/parens/brackets
  ...
  checkWordAt(x, y) {
    ...
    if (doc.caretRangeFromPoint) {
      range = doc.caretRangeFromPoint(xx, yy);
    } else if (doc.caretPositionFromPoint) {
      const pos = doc.caretPositionFromPoint(xx, yy);
      if (pos) {
    ...
    const result = this._expandToWord(range);
    if (result && /^[a-zA-Z]+$/.test(result.word)) {
      ...
      window.flutter_inappwebview.callHandler('onWordTap', result.word, context);
      return true;
    }
    return false;
  }
  ...
  checkSentenceAt(x, y) {
    ...
    if (!range) return false;
    const sentence = this._expandToSentence(range);
  ...
  checkElementAt(x, y) {
    if (this.checkImageAt(x, y)) return;
    this.checkSentenceAt(x, y);
  }
...
/* Custom font override – applied when the user enables "Override Book Font" */
body.lumina-override-font,
body.lumina-override-font * {
  font-family: var(--lumina-font-family, inherit) !important;
}
''';
