...
1. **Dart 类型名**：`EpubImportService` → `BookImportService`、`EpubWebViewHandler` → `BookWebViewHandler`（含 provider、文件与测试重命名）。纯 Dart，无跨语言契约变化。
2. **原生通道方法名**：`pickEpubFiles` / `pickEpubFolder` / `isEpubFile` → `pickBookFiles` / `pickBookFolder` / `isBookFile`，Kotlin、Swift 与 Dart 三处同批改名（通道方法名是跨语言契约，必须一次改齐）。
