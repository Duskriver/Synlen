...
import dk.nota.flutterreadium.EpubTextInteractionBridge
...
class EpubReaderFragment :
    ...
    interface Listener {
        ...
        fun onTextInteraction(json: String, resourceHref: String, sourceView: View) {}
        fun onImageTapped(json: String) {}
        fun onReaderError(code: String) {}
    }

    override fun onResourceLoadFailed(href: org.readium.r2.shared.util.Url, error: org.readium.r2.shared.util.data.ReadError) {
        PluginLog.w(TAG, "Resource load failed: $href: $error")
        lifecycleScope.launch { if (isAdded) listener?.onReaderError("ResourceReadError") }
    }

    var listener: Listener? = null
    var handleInternalLinks = true
    private val injectionAssets = ReadiumReader.javaScriptInjections + ReadiumReader.cssInjections

    override fun shouldFollowInternalLink(
        link: org.readium.r2.shared.publication.Link,
        context: org.readium.r2.navigator.HyperlinkNavigator.LinkContext?,
    ): Boolean = handleInternalLinks || context is org.readium.r2.navigator.HyperlinkNavigator.FootnoteContext
                ...
                .Configuration(
                    ...
                    servedAssets =
                        listOf("flutter_assets/packages/flutter_readium/assets/.*") +
                            injectionAssets.map { asset ->
                                val prefix = asset.packageName?.let { "flutter_assets/packages/$it/" } ?: "flutter_assets/"
                                // PatternMatcher 的简单 glob 中仅 .、*、\\ 是特殊字符。
                                (prefix + asset.assetPath).replace("\\", "\\\\").replace(".", "\\.").replace("*", "\\*")
                            } +
                            model.fontFamilyDeclarations.flatMap { family ->
                                family.faces.map { face -> face.asset }.filterNot { it.startsWith("data:") }
                            },
                    ...
                    model.fontFamilyDeclarations.filter { family -> family.faces.any { !it.asset.startsWith("data:") } }.forEach { family ->
                        addFontFamilyDeclaration(
                            FontFamily(family.name),
                            family.fallbacks.map(::FontFamily),
                        ) {
                            family.faces.filterNot { it.asset.startsWith("data:") }.forEach { face ->
                                addFontFace {
                    ...
                    registerJavascriptInterface("FlutterImageBridge") { _ ->
                        EpubImageTapBridge(publication) { json ->
                            lifecycleScope.launch { if (isAdded) listener?.onImageTapped(json) }
                        }
                    }
                    registerJavascriptInterfaceWithWebView("flutterReadiumText") { link, webView ->
                        EpubTextInteractionBridge { json ->
                            lifecycleScope.launch {
                                if (isAdded) listener?.onTextInteraction(json, link.href.toString(), webView)
                            }
                        }
                    }
                }
