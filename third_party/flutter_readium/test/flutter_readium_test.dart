...
class _RecordingReaderWidget implements ReadiumReaderWidgetInterface {
  final navigations = <({Locator locator, bool animated, bool isAudioBookWithText})>[];

  @override
  Future<void> go(Locator locator, {required bool isAudioBookWithText, bool animated = false}) async {
    navigations.add((locator: locator, animated: animated, isAudioBookWithText: isAudioBookWithText));
  }
...
void main() {
  ...
  group('goToLocator', () {
    test('向当前视口传递完整 Locator 与动画参数，成功返回 true', () async {
      final viewport = _RecordingReaderWidget();
      platform.currentReaderWidget = viewport;
      const locator = Locator(
        ...
        locations: Locations(
          ...
          cssSelector: '#p42',
          progression: .45,
          additionalProperties: {'customLocation': 'keep'},
        ),
        text: LocatorText(highlight: 'Reader position'),
      );
      expect(await reader.goToLocator(locator), isTrue);
      expect(await reader.goToLocator(locator, animated: true), isTrue);
      expect(viewport.navigations.map((call) => call.animated), [false, true]);
      for (final call in viewport.navigations) {
        expect(call.locator.toJson(), locator.toJson());
        expect(call.isAudioBookWithText, isFalse);
      }
    });

    test('没有活动视口时返回 false', () async {
      const locator = Locator(href: '/ch1.xhtml', type: 'application/xhtml+xml');
      expect(await reader.goToLocator(locator), isFalse);
    });
  });
