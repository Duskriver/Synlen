...
VERSION_JSON="$(
  python3 - "$MAJOR" "$MINOR" "$PATCH" "$BUILD_NUMBER" "$TAG" "$UPDATE_LOG" "$BUCKET" "$REGION" <<'PYEOF'
import json, sys
major, minor, patch, build, tag, update_log = (
    int(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3]),
    int(sys.argv[4]), sys.argv[5], sys.argv[6],
)
bucket, region = sys.argv[7], sys.argv[8]
...
# 检查 APK 产物（本地构建在 flutter-apk/，CI 会移动到 build/outputs/）
