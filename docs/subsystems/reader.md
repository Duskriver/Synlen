...
- 点词锚点为 DOM `Range.getClientRects()` 中当前页可见词片段的包围矩形，另传逐片段矩形供词标记使用；原生视口将文档坐标映射为 Flutter 逻辑坐标，宿主传给学习入口前再转换为全局坐标。非法或零面积矩形不触发词卡，阅读边距与设备像素比只在相应坐标边界处理一次。
- 词卡打开期间暂停阅读手势与音量键翻页；页面位置、排版、实际视口几何或应用生命周期变化，以及退出阅读器时关闭词卡。词卡的布局、内容和请求生命周期由 [learning](learning.md#边界与不变量) 持有。
