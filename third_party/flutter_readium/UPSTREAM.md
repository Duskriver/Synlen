...
`ReadiumReaderWidget` 增加 `sessionId`、`onTextInteraction`、`onLocatorChanged`、`onReaderReady`、`onReaderError`、`onReaderDisposed`、`handlePointerControls` 和 `handleInternalLinks`。应用为每次创建提供新的 `sessionId`。文字回执是最多 65536 个 UTF-16 单元的 JSON 字符串，原生层覆盖 `sessionId` 和 `resourceHref`；宿主仍负责校验版本、事件类型、正文长度和当前章节。桥接只承担传输，学习提取逻辑属于应用资源。
