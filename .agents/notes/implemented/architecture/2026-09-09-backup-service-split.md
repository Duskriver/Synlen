...
- `data/services/backup_merger.dart` 提供 `mergeRestoredBook` / `mergeRestoredManifest` 纯函数，保留元数据与进度分别取较新、清单按秒精度比较的语义。事务归属见[藏书写入契约](2026-09-19-library-write-consistency.md)。
...
- 恢复服务注入 `LibraryBookStore`；合并策略与 JSON 映射保持独立。
