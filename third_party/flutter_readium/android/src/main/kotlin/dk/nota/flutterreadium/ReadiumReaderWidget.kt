...
// 位置富化必须限时完成；超时报告失败，不能把初始传入位置冒充实际视口回执。
...
class ReadiumReaderWidget(
    ...
    private var nativeDetached = false
    override val handleInternalLinks = creationParams["handleInternalLinks"] as? Boolean ?: true
    private val sessionId = creationParams["sessionId"] as? String ?: id.toString()
    private val publication = ReadiumReader.currentPublication
    ...
    override fun dispose() {
        ...
        channel.coroutineContext.cancelChildren()
    ...
    init {
        ...
        if (fontResourceFailed) channel.onReaderError("FontResourceUnavailable")
        ...
        launch {
            try {
                ...
                channel.onReaderError("ReaderInitializationFailed")
    ...
    override fun onReaderError(code: String) {
        if (!disposed) channel.onReaderError(code)
    }

    override fun onImageTapped(json: String) {
        if (!disposed) channel.onImageTapped(json)
    }

    override fun onTextInteraction(json: String, resourceHref: String) {
        if (disposed) return
        val payload = runCatching { JSONObject(json) }.getOrNull() ?: return
        payload.put("sessionId", sessionId)
        payload.put("resourceHref", resourceHref)
    ...
    private var locatorRevision = 0L
    ...
    override fun onPageChanged(
        ...
        launch {
            ...
            emitOnPageChanged(pageIndex, totalPages, locator, revision)
    ...
    override fun onVisualCurrentLocationChanged(locator: Locator) {
        ...
        if (disposed || !hasSentReady) return
        // StateFlow 首值可能只是传入的 initialLocator；实际落位由 onPageChanged 回执。
    ...
    private suspend fun emitOnPageChanged(
        ...
        try {
            // 页面信息、资源身份和短文本在同一次 JS 执行中采样，切页后丢弃旧任务。
            var staleResource = false
            val enriched =
                    ...
                    when {
                        ...
                        ReadiumReader.isComic -> {
                            ...
                            snapshot.optJSONObject("anchor")?.let { anchor ->
                                val selector = anchor.optJSONObject("locations")?.optString("cssSelector")
            ...
            if (disposed || revision != locatorRevision || staleResource) return
            if (enriched == null) {
                PluginLog.w(
                    TAG,
            ...
            if (!disposed && revision == locatorRevision) channel.onReaderError("LocatorUnavailable")
        }
    }
    ...
    override fun onMethodCall(
        ...
        if (disposed && call.method != "dispose") {
            result.error("ReaderClosed", "Reader is closed", null)
            return
        }
