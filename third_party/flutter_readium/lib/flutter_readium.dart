...
class FlutterReadium {
  ...
  Future<void> goBackward({bool animated = true}) => _readiumCall(() async {
    // ignore: invalid_use_of_protected_member
    await _platform.currentReaderWidget?.goBackward(animated: animated);
  });

  /// Navigates forward in the publication, such as to the next page or section.
  ///
  /// The exact behavior may depend on the publication's format and if audio or TTS is enabled.
  Future<void> goForward({bool animated = true}) => _readiumCall(() async {
    // ignore: invalid_use_of_protected_member
    await _platform.currentReaderWidget?.goForward(animated: animated);
  });
  ...
  Future<bool> goToLocator(Locator locator, {bool animated = false}) => _readiumCall(() async {
    // ignore: invalid_use_of_protected_member
    final reader = _platform.currentReaderWidget;
    if (reader == null) return false;
    await reader.go(locator, animated: animated, isAudioBookWithText: false);
    return true;
  });
