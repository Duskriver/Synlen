...
| 7 个 part mixin | `spine_navigation` / `page_navigation` / `progress` / `theme` / `link_handling` / `image_viewer` / `footnote` | `lib/src/features/reader/presentation/mixins/` |
...
3. 导航：`planChapterPreload` 给出当前 / 上一章 / 下一章三个请求 → `ReaderRenderer` 装载三 iframe；翻页边界由 `resolvePageTurnBoundary` 判定，章内目标页由 `pageTurnTargetIndex` 给出，越界则跨章。
...
- `reader/presentation` 仍是最大的 UI 层，7 个 part mixin 的逻辑继续下沉 `reader/application`。
- 7 个 mixin 是 part 文件（依赖 `reader_screen.dart`），单测需先拆分或改 widget test（[测试现状](../testing.md#现状)）。
