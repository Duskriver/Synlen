...
class _ReaderScreenState extends ConsumerState<ReaderScreen>
  ...
  String? _wordPopupSession;
  Object? _wordPopupLocation;
  ...
  void initState() {
    ...
    _settingsSubscription = ref.listenManual(readerSettingsProvider, (
      ...
      if (mounted && _started) {
        ...
        if (layout != session.layout) _dismissWordPopup?.call();
  ...
  void _onSessionChanged() {
    if (!mounted) return;
    if (_dismissWordPopup != null &&
        (!session.ready ||
            session.sessionId != _wordPopupSession ||
            _location != _wordPopupLocation)) {
      _dismissWordPopup?.call();
    }
    _syncVolume();
  ...
  // 文本上下文补齐不代表翻页，只比较位置字段。
  Object get _location => (
    session.locator?.href,
    session.locator?.locations?.progression,
    session.locator?.locations?.position,
    session.locator?.locations?.toJson()['currentPage'],
  );
  ...
  Future<void> _interaction(ReadiumInteraction event, Rect? anchorRect) async {
    ...
    switch (event.kind) {
      ...
      case 'word':
        ...
        _wordPopupSession = session.sessionId;
        _wordPopupLocation = _location;
        try {
