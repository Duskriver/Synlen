...
class EpubNavigator :
    ...
    /** 本地字体在 HTML 首次排版之前注入，避免加载完成后换字体造成位置漂移。 */
    internal fun localFontStyleSheet(): String = fontFamilyDeclarations.flatMap { family ->
        family.faces.filter { it.asset.startsWith("data:") }.map { face ->
            val name = JSONObject.quote(family.name).replace("<", "\\3c ")
            val style = if (face.style == dk.nota.flutterreadium.ReaderFontStyle.ITALIC) "italic" else "normal"
            "@font-face { font-family:$name; src:url(${JSONObject.quote(face.asset)}); font-style:$style; font-weight:${face.weight}; }"
        }
    }.joinToString("\n")
    ...
    interface VisualListener {
        val handleInternalLinks: Boolean get() = true
        /**
    ...
    override suspend fun initNavigator() {
        epubNavigator =
            EpubReaderFragment().apply {
                handleInternalLinks = visualListener.handleInternalLinks
                vm =
    ...
    override fun onReaderError(code: String) {
        visualListener.onReaderError(code)
    }

    override fun onImageTapped(json: String) {
        visualListener.onImageTapped(json)
    }

    override fun onTextInteraction(json: String, resourceHref: String) {
        visualListener.onTextInteraction(json, resourceHref)
    }
    ...
    override fun dispose() {
        ...
        // Flutter 平台通道在主线程调用；回包之前同步拆除，防止旧视口关闭新会话。
        epubNavigator?.let { fragment ->
            fragment.listener = null
            // 此视口已关闭，不需恢复 fragment；后台保存状态后也必须完成拆除。
            if (fragment.isAdded) fragment.parentFragmentManager.commitNow(allowStateLoss = true) { remove(fragment) }
        }
